#tag WebPage
Begin WebPage CVEScoreDatePage
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  ...
		  SqlDataListBox.HeaderAt(1) = "CVE"
		  SqlDataListBox.HeaderAt(2) = "CVSSv3.x Score"
		  SqlDataListBox.HeaderAt(3) = "CVSSv3.x Vector String"
		  SqlDataListBox.HeaderAt(4) = "CVSSv2 Score"
		  SqlDataListBox.HeaderAt(5) = "CVSSv2 Vector String"
		  SqlDataListBox.HeaderAt(6) = "Published Date"
		End Sub
	#tag EndEvent
	...
	#tag Method, Flags = &h0
		Sub Populate(currentpage As Integer)
		  ...
		  For i=1 to tmpPageRange
		    ...
		    Session.sqlData.ColumnAt(3).StringValue.TrimRight, _
		    Session.sqlData.ColumnAt(4).StringValue.TrimRight, _
		    Session.sqlData.ColumnAt(5).StringValue.TrimRight)
		    Session.sqlData.MoveToNextRow
		  next
		  ...
		  CVEScoreDatePage.UpdateBrowser
...
#tag Events SearchButton
	#tag Event
		Sub Pressed()
		  ...
		  mysql = "SELECT cve, base_score_3, vector_string_3, base_score, vector_string, published_date FROM cvss WHERE (base_score_3 >= "+ score + "OR base_score >= "+ score+") AND (published_date >= '"+date+"'::date)"
		  Try
...
#tag Events ExportButton
	#tag Event
		Sub Pressed()
		  ...
		  mTextFile.FileName = "CVEexport.csv"
		  
		  dim fstring As string
		  
		  Session.sqlData.MoveToFirstRow
		  fstring = "CVE,CVSSv3 Score,CVSSv3 Vector String,CVSSv2 Score,CVSSv2 Vector String,Published Date" + EndOfLine
		  ...
		  for Each row as DatabaseRow In Session.sqlData
		    ...
		    Session.sqlData.ColumnAt(3).StringValue.TrimRight + "," + _
		    Session.sqlData.ColumnAt(4).StringValue.TrimRight + "," + _
		    Session.sqlData.ColumnAt(5).StringValue.TrimRight + EndOfLine
